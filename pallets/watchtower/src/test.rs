...
use super::mock::*;
use crate::{Error, Event as WatchtowerEvent, NodeManagerInterface, SummarySourceInstance, WatchtowerSummaryStatus};
...
fn submit_watchtower_vote_works() {
    ExtBuilder::build_default()
        ...
        .execute_with(|| {
            ...
            assert_watchtower_vote_event_emitted(
...
fn voting_period_update_works() {
    ExtBuilder::build_default()
        ...
        .execute_with(|| {
            ...
            assert_ok!(Watchtower::set_voting_period(
                RuntimeOrigin::root(),
                new_period
            ));
    ...
    ExtBuilder::build_default()
        ...
        .build_and_execute_with_state(|ext, _pool_state, _offchain_state| {
            ext.execute_with(|| {
                ...
                let signing_key = MockNodeManager::get_node_signing_key(&watchtower_account_1()).unwrap();
                ...
                let wrong_data = (crate::WATCHTOWER_OCW_CONTEXT, &instance, &root_id, !vote_is_valid);
                assert!(!Watchtower::offchain_signature_is_valid(
                    &wrong_data,
                    &signing_key,
                    &signature
...
fn ocw_response_validation_works() {
    ExtBuilder::build_default()
        ...
        .execute_with(|| {
            ...
            let valid_response = b"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef".to_vec();
            ...
            let invalid_hex_response = b"gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg".to_vec();
